...
interface CoachNotesSessionRow {
    id: string;
}
...
async function resolveMethodStep(
    ...
    if (input.stepOrder === null) {
        throw new ConflictError("L'étape de méthode est requise pour ces notes.");
    }
...
async function resolveEvaluationSession(
    adminSupabase: ReturnType<typeof createAdminClient>,
    roleplayId: string,
    userId: string,
    sessionId: string,
    ...
    const { data, error } = await adminSupabase
        ...
        .eq("user_id", userId)
        .maybeSingle<CoachNotesSessionRow>();

    if (error) throw error;
    if (!data) throw new NotFoundError("Session de roleplay introuvable.");
...
async function resolveCoachNotesContext(
    ...
    if (input.sessionId) {
        const evaluationSession = await resolveEvaluationSession(
            adminSupabase,
            roleplay.id,
            context.userId,
            input.sessionId,
        ...
        return { adminSupabase, context, evaluationSession, methodStep: null, roleplay };
    }

    const methodStep = await resolveMethodStep(adminSupabase, roleplay.method_id, input);

    return { adminSupabase, context, evaluationSession: null, methodStep, roleplay };
}
...
export async function listRoleplayCoachNotes(
    ...
    const { data, error } = await adminSupabase
        ...
        .select("id, notes, saved_at, coach_mode, method_step_id, session_id, step_order")
...
export async function getRoleplayCoachNotes(
    ...
    const { adminSupabase, context, evaluationSession, methodStep, roleplay } = await resolveCoachNotesContext(roleplayId, input);
    let query = adminSupabase
        .from("roleplay_coach_notes")
    ...
    query = evaluationSession
        ? query.eq("session_id", evaluationSession.id).is("method_step_id", null)
        : query.eq("method_step_id", methodStep!.id).is("session_id", null);

    const { data, error } = await query.maybeSingle<CoachNotesRow>();
...
export async function saveRoleplayCoachNotes(
    ...
    existingQuery = evaluationSession
        ? existingQuery.eq("session_id", evaluationSession.id).is("method_step_id", null)
        : existingQuery.eq("method_step_id", methodStep!.id).is("session_id", null);

    const { data: existing, error: existingError } = await existingQuery.maybeSingle<CoachNotesRow>();
    ...
    if (existing) {
        const { error } = await adminSupabase
            ...
            .update({
                ...
                method_step_id: methodStep?.id ?? null,
                notes: input.notes,
                saved_at: savedAt,
                session_id: evaluationSession?.id ?? null,
                step_order: methodStep?.step_order ?? 1,
                updated_at: savedAt,
            })
            .eq("id", existing.id);

        if (error) throw error;
        revalidateRoleplayCoachNoteConsumers(roleplay.id, evaluationSession?.id ?? null);
        return { savedAt };
    }
